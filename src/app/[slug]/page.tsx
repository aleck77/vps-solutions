...
export async function generateStaticParams() {
  // Guard: без Firebase конфига — пустой массив, страницы рендерятся в рантайме (ISR/SSR)
  if (!process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID) {
    console.warn('[generateStaticParams][slug] Firebase not configured — skipping static generation');
    return [];
  }
  ...
  try {
    ...
    return pagesSnapshot.docs.map((doc) => ({ slug: doc.id }));
  } catch (error) {
    console.error('[generateStaticParams][slug] Error fetching pages:', error);
    return []; // Не роняем билд, страницы отдадутся динамически
  }
}
